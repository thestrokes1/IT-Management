...
class AssetService(EventPublisher):
    ...
    def create_asset(
        ...
        # Handle serial_number: generate unique one if not provided
        if serial_number and serial_number.strip():
            ...
            serial_number = f"AST-{uuid.uuid4().hex[:12].upper()}"
        ...
        # Create asset with proper error handling
        try:
            ...
            if 'serial_number' in error_msg:
                ...
                raise ValidationError(
                    message=f"Failed to create asset due to database constraint: {str(e)}",
                    field='name'
                )
...
__all__ = ['TicketService', 'AssetService']
